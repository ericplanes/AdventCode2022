...
fs.readFile(process.env.PWD + "/data.txt", function (err, data) {
  ...
  movements.forEach((e) => {
    ...
    source = e[1] - 1;
    destination = e[2] - 1;
    length = stack[source].length;
    stack[destination].push(...stack[source].slice(length - amount, length));
    stack[source] = stack[source].slice(0, length - amount);
  });
  console.log(stack);
